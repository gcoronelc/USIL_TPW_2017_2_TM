...
public class CursoService implements CursoServiceSpec{
  ...
  @Override
  public List<Map<String, Object>> leerDetalle(String ciclo, String curso) {
    List<Map<String, Object>> lista;
    // Inicio de proceso
    String sql = "select IdCursoProg cursoprog, IdCiclo ciclo, "
        + "IdCurso curso, Vacantes disponibles, matriculados,"
        + "horario, IdProfesor profesor, activo "
        + "from CursoProgramado "
        + "where IdCiclo = ? and IdCurso = ? ";
    lista = jdbcTemplate.queryForList(sql, ciclo, curso);
    // Fin de proceso
    return lista;
  }
